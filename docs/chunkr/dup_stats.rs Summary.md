...
**Purpose**: Analyzes duplication statistics from duplicate reports to quantify
redundant data **Key functionality**:
...
- Outputs human-readable summaries (top 5 duplicates) or machine-readable JSON
  **Critical flow**: Read report → Parse groups → Calculate stats → Format
