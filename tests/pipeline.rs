...
#[tokio::test]
#[ignore]
async fn chunk_and_insert_pipeline()
-> Result<()> {
  ...
  let example_root = config
    ...
    .clone()
    .unwrap_or_else(|| {
      PathBuf::from("examples/cfr")
    });
  let sample_files =
    list_example_files(&example_root)?;
  if sample_files.is_empty() {
    return Err(anyhow!(
      "no example .txt files found \
       under {}",
  ...
  config.paths.extract_root =
    extract_root.clone();
  config.paths.chunk_root =
    chunk_root.clone();
  config.paths.state_dir =
    state_dir.clone();

  let embed_dim = detect_embedding_dim(
    &client,
    &config.insert.embeddings
  )
  .await?;
  config.insert.qdrant.vector_size =
    embed_dim;
  ...
  let test_collection = config
    .insert
    .qdrant
    .collection
    .clone();
  let test_index = config
    .insert
    .quickwit
    .index_id
    .clone();
  ...
  .await?;
  info!(
    "[test] chunk finished in {:?}, \
     building sample query",
    started.elapsed()
  );
  let sample_query =
    sample_query_from_chunks(
      &chunk_root
    )?;
  info!(
    "[test] sample query picked \
     (len={}): {:?}",
  ...
  .await?;
  info!(
    "[test] insert finished in {:?}, \
     verifying qdrant/quickwit",
    started.elapsed()
  );
...
async fn reset_quickwit(
  client: &Client,
  url: &str,
  index_id: &str
) -> Result<()> {
  let _ = delete_quickwit(
    client, url, index_id
  )
  .await;
  let create_url = format!(
...
async fn verify_qdrant(
  ...
  if count == 0 {
    return Err(anyhow!(
      "qdrant count is zero"
    ));
  }

  let embed = ollama_embed(
    client, embeddings, query
  )
  .await?;
  ...
  if hits == 0 {
    return Err(anyhow!(
      "qdrant search returned no hits"
    ));
  }
  Ok(())
}

async fn verify_quickwit(
  client: &Client,
  url: &str,
  index_id: &str,
  query: &str
...
fn copy_truncated(
  ...
  fs::write(dst, slice).with_context(
    || {
      format!("write {}", dst.display())
    }
  )?;
  Ok(())
}

fn list_example_files(
  root: &Path
) -> Result<Vec<PathBuf>> {
...
fn sample_query_from_chunks(
  chunk_root: &Path
) -> Result<SampleQuery> {
  for entry in
    walkdir::WalkDir::new(chunk_root)
      .into_iter()
      .filter_map(|e| e.ok())
      .filter(|e| {
        e.file_type().is_file()
      })
  {
      ...
      .find(|l| !l.trim().is_empty())
    {
        ...
        .and_then(|v| v.as_str())
      {
        ...
        if !snippet.is_empty() {
          let embed_text: String =
            snippet
              .chars()
              .take(160)
              .collect();
          let term = pick_query_term(
            &embed_text
          )
          .unwrap_or_else(|| {
            embed_text.clone()
  ...
  Err(anyhow!(
    "no chunk text available for query"
  ))
}
