...
fn chunk_file(
  ...
  let rel = path
    ...
    .unwrap_or(path);
  let out_path = replace_extension(
    &config.paths.chunk_root.join(rel),
    "jsonl"
  );
  if let Some(parent) =
    out_path.parent()
  ...
  let cleaned =
    normalize_text(&raw, &config.chunk);
...
fn normalize_text(
  ...
  if cfg.normalize_unicode {
    out =
      out.nfkc().collect::<String>();
  }
  if cfg.collapse_whitespace {
    let mut collapsed =
      String::with_capacity(out.len());
      ...
      for part in bounded_parts {
          ...
          && !current.is_empty()
        {
          if !last_overlap.is_empty() {
            let mut overlap_chunk =
              last_overlap.clone();
            overlap_chunk.push(' ');
            overlap_chunk
              .push_str(&part);
            if overlap_chunk.len()
              > cfg.max_chunk_chars
            {
              current.clear();
...
fn should_include_metadata(
  key: &str,
  cfg: &ChunkConfig
) -> bool {
  match key {
    | "calibre_id" => {
      cfg.metadata.include_calibre_id
    }
    | "title" => {
      cfg.metadata.include_title
    }
    | "authors" => {
      cfg.metadata.include_authors
    }
    | "published" => {
      cfg.metadata.include_published
    }
    | "language" => {
      cfg.metadata.include_language
    }
    | _ => true
  }
}
