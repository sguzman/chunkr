...
pub async fn run(
  ...
  if total_files == 0 {
    warn!(
      "no chunk files found for insert"
    );
    return Ok(());
  }
  info!(
    total_files,
    max_parallel_files =
      config.insert.max_parallel_files,
    "insert starting"
  );
  ...
  let global_embed_limit = if config
    .insert
    .embeddings
